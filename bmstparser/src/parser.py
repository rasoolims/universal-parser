...
if __name__ == '__main__':
    ...
    if options.predictFlag:
        ...
        with open(options.netfile, 'r') as paramsfp:
            chars, net_options, deep_lstm_params, char_lstm_params, clookup_params, proj_mat_params, plookup_params = pickle.load(paramsfp)
        ...
        print 'Initializing lstm mstparser:'
        parser = mstlstm.MSTParserLSTM(universal_tags, rels, options, chars, deep_lstm_params, char_lstm_params, clookup_params, proj_mat_params, plookup_params, net_options)
        best_acc = -float('inf')
        t, epoch = 0,1
