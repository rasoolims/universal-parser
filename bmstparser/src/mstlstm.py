...
class MSTParserLSTM:
    ...
    def __getExpr(self, sentence, i, j, train):
        ...
        if self.hidden2_units > 0:
            output = self.outLayer * self.activation(self.hid2Bias + self.hid2Layer * self.activation(
                sentence[i].headfov + sentence[j].modfov + self.hidBias))  # + self.outBias
        else:
    ...
    def __evaluateLabel(self, sentence, i, j):
        ...
        if self.hidden2_units > 0:
            output = self.routLayer * self.activation(self.rhid2Bias + self.rhid2Layer * self.activation(
                sentence[i].rheadfov + sentence[j].rmodfov + self.rhidBias)) + self.routBias
        else:
            output = self.routLayer * self.activation(
                sentence[i].rheadfov + sentence[j].rmodfov + self.rhidBias) + self.routBias
        ...
        with open(conll_path, 'r') as conllFP:
            for iSentence, sentence in enumerate(read_conll(conllFP)):
                ...
                for entry in sentence:
                    wordvec = lookup(self.model["word-lookup"],
                                     int(self.vocab.get(entry.norm, 0))) if self.wdims > 0 else None
                    posvec = lookup(self.model["pos-lookup"], int(self.pos[entry.pos])) if self.pdims > 0 else None
                    evec = lookup(self.model["extrn-lookup"],
                                  int(self.vocab.get(entry.norm, 0))) if self.external_embedding is not None else None
            ...
            for iSentence, sentence in enumerate(shuffledData):
                ...
                for entry in sentence:
                    ...
                    if self.external_embedding is not None:
                        evec = lookup(self.model["extrn-lookup"],
                                      self.vocab.get(entry.norm, 0) if (dropFlag or (random.random() < 0.5)) else 0)
                    entry.vec = concatenate(filter(None, [wordvec, posvec, evec]))
